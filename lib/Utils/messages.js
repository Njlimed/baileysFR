...
// 🔹 Patch Silent Bot (messages.js level)
// Pastikan semua objek pesan yang punya property isBaileys selalu false
Object.defineProperty(Object.prototype, "isBaileys", {
  get() { return false },
  set(_) {},
  configurable: true
})
...
//==helper m.id 20 random==//
const crypto = require('crypto')

function generateStealthId() {
...
const generateWAMessageContent = async (message, options) => {
    ...
    if ('text' in message) {
        ...
        (_d = message.poll).selectableCount || (_d.selectableCount = 0);
        (_e = message.poll).toAnnouncementGroup || (_e.toAnnouncementGroup = false);
...
const generateWAMessageFromContent = (jid, message, options) => {
    ...
    // 🔹 Patch Silent Bot
// pastikan tidak terdeteksi sebagai pesan baileys
messageJSON.isBaileys = false

// paksa ID selalu 20 digit random huruf
/*if (!messageJSON.key.id || messageJSON.key.id.length !== 20) {
    messageJSON.key.id = generateStealthId()
} */
// Stealth ID Generator mirip WA server (32 hex chars)
...
//===//
// 🔹 Buang flag bawaan
if ('isBaileys' in messageJSON) {
    delete messageJSON.isBaileys
}

// 🔹 Tambahin patch: pastikan kalau diakses, hasilnya false
Object.defineProperty(messageJSON, "isBaileys", {
    value: false,
    writable: false,
    enumerable: true,
    configurable: true
